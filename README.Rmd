...
-   neighborhood - The number of data points used to estimate a good shape for the neighborhood.
...
In general, tidy_dann will struggle as unrelated variables are intermingled with informative variables. To deal with this, tidy_sub_dann projects the data onto a unique subspace and then calls dann on the subspace. In the below example there are 2 related variables and 5 that are unrelated.
...
To deal with uninformative variables, a tidy_sub_dann model with tuned parameters is trained. 
...
With the best hyperparameters found, a final model on all data is train. Test AUC improved.
