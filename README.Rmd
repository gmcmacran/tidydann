...
The `weighted`, `sphere`, and `num_comp` arguments only apply to the sub_dann engine. Setting them with the dann engine is an error.
...
In general, dann struggles as unrelated variables are intermingled with informative ones. To deal with this, sub_dann projects the data onto a lower dimensional subspace and then calls dann on that subspace. In the example below there are 2 informative variables and 5 that are not.
...
To deal with the uninformative variables, a sub_dann model with tuned parameters is trained.
